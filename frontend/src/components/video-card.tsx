...
export function VideoCard({ video }: VideoCardProps) {
  return (
    <div className="group rounded-lg border border-blue-100 bg-white hover:shadow-sm transition-shadow">
      <Link href={`/watch/${video.id}`} className="block relative overflow-hidden rounded-t-lg">
      ...
      <div className="flex gap-3 p-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <div className="mt-0.5 text-xs text-muted-foreground">
            <Link href={`/channel/${channelSlug(video.channel)}`} className="hover:text-blue-700">
              {video.channel}
            </Link>
          </div>
